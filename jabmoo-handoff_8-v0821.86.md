...
> อัปเดตล่าสุด: **prod v0821.86 / dev v0821.86-dev** (2026-09-19) — **prod = dev ทุกบรรทัด ยกเว้น 6 บรรทัด dev-only** · A push เองผ่าน GitHub Desktop
...
|---|---|---|
| jabmoo.html | **v0821.86** | prod — COIN_ON=true, ~1.52MB |
| jabmoo-dev.html | **v0821.86-dev** | ต่าง prod **6 บรรทัด**: manifest-dev/icon-dev/ชื่อแอป DEV + `__JM_DEV=true` + VER + COIN_ON gate |
...
## 5. BOT — ตัวละคร + อาวุธ
`BOT_CHARS` knobs: dealsX, blur, hunt, campaign+huntP, antiLast, blowGuard, bayes, slamDef, hlak, **spGamble**, **pigLead (.86, default 1 ทุกตัว)** — override ทาง game_config
...
- **PIG TOP GUARD + VOID SEED (.86, A's call G1 T2 2026-09-20)** ใน botPlayMC ตอนนำ + ถือ Q♠: (1) prune นำ A♠ (และ K♠ เมื่อ A♠ หมด) ถ้าโพดำ <6 ใบ — กัน Q♠ กลายเป็นโพดำสูงสุด (2) มี singleton นอกโพดำ (ไม่ใช่ 10♣/J♦) ที่ยังมีใบสูงกว่า live ≥3 → บังคับนำ singleton นั้นสร้างช่องทิ้ง Q♠ · bench aa n=150 (deals 30/80ms) **+1.4±5.0 = กลาง ไม่ถดถอย** hits 50+54/300 เกม · repro T2: ปิดกฎ MC เลือก 5♥ 40% A♠ 7%, เปิด = 5♥ 100% · ปิดได้ `{"chars":{"thepjab":{"pigLead":0}}}` (ทุกตัว)
...
- ปุ่มเล่น เขียว+ขอบทอง · ปุ่ม 😊 อิโมจิ (14 ตัว, .83 +🙂😄; grid 4 คอลัมน์ portrait / 7 landscape) + ปุ่ม 💬 แชตกลม (online เท่านั้น) มุมล่างขวาผ้า
- **แผงแชตบนผ้า (.76, portrait)**: แทน drawer ล่าง (โค้ดเก่าปิดด้วย `false&&`) · อยู่ขวาผ้า top=`max(ใต้บล็อกที่นั่งขวา(rightSeatRef), ใต้ไพ่ที่ลงขวา)` bottom=54 (เหนือปุ่ม 💬/😊) left=`max(50%+tW2/2+8, 100%−196px)` ไม่ทับไพ่ตัวเอง · online เท่านั้น · จาง (opacity .55) จนแตะ/โฟกัส input → เต็ม (chatOpen) · blur → จางกลับ · ซ่อนตอน roundSummary · landscape ยัง drawer เดิม · **TYPING MODE (.78/.79)**: `chatOpen` = โหมดพิมพ์ → root สูง = `visualViewport.height` (พ้นคีย์บอร์ด) · header เต็มความกว้างเหมือนเดิม · แถวกลาง = [ผ้าโต๊ะ wrapper 60% (ผ้าข้างใน 166.7% แล้ว `scale(0.6)` มุมซ้ายบน → layout ผ้าเหมือนปกติ) | คอลัมน์แชต 40% สูงเท่าแถวกลาง input `autoFocus`] · แผงมือไพ่เต็มขนาด: ไพ่ + ปุ่มเล่น 90px อยู่ขวาไพ่แถวเดียว (`avail=innerWidth−112`) · **(.80) คีย์บอร์ดค้างตอนลงไพ่**: root `onMouseDown` preventDefault ทุกที่นอกคอลัมน์แชต (`chatColRef`) + input `onBlur` → refocus ถ้ายัง typing · คีย์บอร์ดหายเฉพาะกด 💬/✕ · **(.81) ตอน typing: `tz=1.15` → ไพ่ที่ลง (tW2/tH2) + ไพ่แต้ม (`<SCP sc={tz}>`) ใหญ่ขึ้น 15%** · **(.82) iOS ไม่ยอมให้กันคีย์บอร์ดหาย (mousedown preventDefault ไม่พอ) → เปลี่ยนกลยุทธ์: layout ไม่ขึ้นกับคีย์บอร์ด** — root สูง = `typingHRef` (vv ต่ำสุดที่เคยเห็นตั้งแต่เข้าโหมด = สูงตอนคีย์บอร์ดขึ้น) คงไว้แม้คีย์บอร์ดหาย · `vh` ของ portrait freeze ใน `vhFrozenRef` ตอน typing (k/sf ไม่เด้ง ไพ่ไม่เปลี่ยนขนาด) · root `onClickCapture` → ถ้า input ไม่โฟกัส refocus ในเจสเจอร์ (iOS อนุญาต) = แตะไพ่/อะไรก็ได้เรียกคีย์บอร์ดกลับ · แผงบนผ้าซ่อนตอนพิมพ์ (ช่องล่างของแผงเป็น div แตะเข้าโหมด ไม่ใช่ input จริง) · ออกด้วย ✕ หรือปุ่ม 💬 · vv resize/scroll → `scrollTo(0,0)` กันหน้าเด้ง · **`CHAT_SOLO` (.77) = `!!window.__JM_DEV` บรรทัดเดียวกันทั้ง 2 ไฟล์ → dev โชว์แผง+ปุ่ม 💬 ใน vs BOT ด้วย (ทดสอบคนเดียว) prod ยัง online เท่านั้น**
...
**📋 copy log (.85)**: `recentLog(2)` = 2 เกมล่าสุดตาม header `=== GAME N` (เกมที่จบ + เกมใหม่ที่เพิ่งเริ่ม) ใช้ทั้งปุ่มในเกม portrait/landscape และ `getLog` ของ GameOver · log เต็มยังอยู่ใน `gameLogRef` (resume ใช้)
...
- **AUTO-REJOIN (.75/.84)**: client เห็น frame ที่ `sub[mySeat]=true` ขณะยังต่ออยู่ → ส่ง `rejoin` เอง (throttle 3s) · host รับ `play` จากที่นั่งที่ subbed → un-sub แล้วรับไพ่ · **(.84) บั๊ก .75: transport dispatch (`m.type==="play"||"again"||"leave"`) ไม่ forward "rejoin" → ไม่เคยทำงาน แก้แล้ว** · **(.84) Overlays: แบนเนอร์แดง "🤖 บอทกำลังเล่นแทนคุณ — แตะเพื่อกลับ" ฝั่ง client เมื่อ `subView[mySeat]` แตะ = ส่ง rejoin** · ⚠️ ต้องอัปทั้ง host และ client ถึงจะหาย — เคส A: A=client (dev ใหม่) แต่ Gung=host (prod) → ต้อง push prod ด้วย
...
- **แชตในเกม (.68/.70)**: บับเบิลข้างที่นั่งคนส่ง 4.5 วิ (อิโมจิ 2.2 วิ) ทั้ง portrait/landscape · ตัดที่ 90 ตัวอักษร สูงสุด 4 บับเบิล · **(.83) host relay `gchat`/`gemote` กลับไปหาคนส่งด้วย → client เห็นข้อความตัวเอง 2 ครั้ง — แก้ที่ `bindChat` ข้าม `m.id===myId` / `bindEmote` ข้าม `m.seat===mySeat`**
...
5. **(พักไว้ A ยังไม่ตัดสินใจ 2026-09-20) บอทเรียนรู้จากคน: เก็บ `play_log` ลง Supabase → วิเคราะห์ปรับ knob ทาง game_config → โปรไฟล์ต่อคน** · Bayesian ขยาย · botChar ลง coin_match seats meta · รูปหมูตัวที่ 5 · ลงโทษ host หนี · per-player min online / Elo / Edge Function / สามกอง (project แยก)
...
- เครื่องมือ (sandbox รีเซ็ตทุกแชท สร้างใหม่จากคำอธิบาย): extract.py / bench.js (`--mode aa|camp|hlak|slamdef|slamrate`) / trace.js / mount.js · **(.86 สร้างใหม่แบบเบา)**: `extract.js` (babel transform script → engine_raw.js) → `load.js` (รันใน node vm พร้อม stub window/document/React/localStorage, ตัด createRoot, export `{gReduce,botPlayMC,botChar,calcScore,...}`) → `bench2.js N DEALS MS CHAR` (aa: deal เดียวเล่น 2 รอบ on=[1,0,1,0]/[0,1,0,1] วัด on−off; seed deal ด้วย mulberry32) ~3.4s/คู่ที่ deals 30 · `repro.js` เรียก botPlayMC ตรงๆ ด้วยมือไพ่จาก log นับการเลือก 30 ครั้ง
...
- A ชอบเห็น preview ภาพก่อนตัดสินใจ UI — ทำ screenshot เทียบให้เสมอ **และต้อง present_files ภาพนั้นด้วย (view เห็นแค่ฝั่ง Claude)**
- **เมื่อวิเคราะห์ log ไพ่ (A สั่ง 2026-09-21): วิเคราะห์จากข้อมูลที่ผู้เล่นคนนั้น "เห็นได้จริง ณ ตอนนั้น" เท่านั้น** (ไพ่ที่ลงไปแล้ว, void ที่เปิดเผย, มือตัวเอง) **ห้ามตอบแบบเห็นไพ่ครบ 13 ใบทุกคน** และ **ห้ามตอบมักง่ายว่า "ทางไหนผลก็เท่ากัน"** — ให้ reproduce ด้วย engine จาก state จริง (repro3/4.js: สร้าง S จาก log ผ่าน gReduce แล้วเรียก botPlayMC/ดู MC vals ต่อใบ) แล้วอธิบายด้วยตัวเลขจริง · บทเรียน G3 T10 J♦: เหตุผลจริงคือ 10♣ คูณสอง (58% ที่หมูเทพถือ K/A♦) ไม่ใช่ "รู้ว่า A ถือ K♦A♦"
- **แชทยาว: พอรู้สึกว่าใกล้ตัน ให้สร้าง handoff ใหม่ทันที อย่ารอจบงาน**

## 13. ประวัติ .35-.72
.35-.41 โต๊ะใหม่ (TABLE_BG, 4 ที่นั่ง, ไพ่ชิดขอบ, poster, ไอคอน PWA) | .42 วัดบล็อกที่นั่งจาก DOM | .43-.47 iOS viewport | .46 HoF โพเดียม | .48-.51 สรุปคะแนนกล่องตามที่นั่ง + อิโมจิ 12 + hold 5 วิ | .52-.53 GameOver ไม่ล้นจอ + landing safe-area | .54 corner index + color-scheme dark | .55-.56 (dev only ถอนออก .60) | .57-.59 หมูเจ้าหญิง avatar id3 | .60-.61 corner index ตามจำนวนใบ | .62 ⚡ จบเร็ว | .63 ไพ่แต้มแถวตัวเอง | .64 SPADE HIGH GAMBLE | .65 rematch ready vote | .66 online rejoin + AudioContext เดียว | .67 ชื่อบอทซ้ำเติมเลข | .68 แชตบับเบิล | .69 ปุ่มกลับห้องเดิม | .70 ปุ่มแชตกลม | **.71 countdown กลางโต๊ะ + เสียงบี๊บดังขึ้น** | **.72 ไอคอนหมูแว่น + เกม N/M + landscape รื้อใหม่ธีมเขียว-ทอง** | **.73 header ในเกมใช้ไอคอนแอปจริง** | **.74 landscape safe-area ซ้าย/ขวา (--sal) + หน้าจบเกมแนวนอน** | **.75 fastFinish ไม่ค้างข้าม rematch + client auto-rejoin เมื่อถูก sub** | **.76 แผงแชตใสบนผ้า portrait (แทน drawer)** | **.77 CHAT_SOLO: dev เห็นแชตใน vs BOT** | **.78 typing mode: โต๊ะย่อ 60% + คอลัมน์แชตสูงเท่า visualViewport** | **.79 typing mode: header/มือไพ่เต็มขนาด ย่อเฉพาะผ้า ปุ่มเล่นขวาไพ่** | **.80 ลงไพ่ระหว่างพิมพ์ คีย์บอร์ดไม่หาย** | **.81 typing: ไพ่ที่ลง+ไพ่แต้ม +15%** | **.82 typing layout คงที่แม้คีย์บอร์ดหาย + แตะไหนก็เรียกคีย์บอร์ดกลับ** | **.83 แชตซ้ำ 2 ครั้งฝั่ง client (echo) + อิโมจิ 🙂😄** | **.84 rejoin ถูก transport ทิ้ง (แก้) + แบนเนอร์บอทเล่นแทน** | **.85 📋 copy 2 เกมล่าสุด** | **.86 PIG TOP GUARD + VOID SEED (bench กลาง)**
